...
Race_SCHOOL_ONLY <- read_excel("school_stats_data/Race SCHOOL ONLY.xlsx")
race <- read_excel("school_stats_data/race.xlsx")
race_diff <- read_excel("school_stats_data/race diff.xlsx")
poc_per_school <- read_excel("school_stats_data/poc per school.xlsx")
funding <- read_excel("school_stats_data/funding.xlsx")
all_race <- read_excel("school_stats_data/all race.xlsx")
schoolstats <- read.csv("school_stats_data/Data + School Info - School Statistics.csv")

#Load Map Data
durham <- geojsonio::geojson_read("map_data/Ten Schools.geojson", what = "sp")
cc <- geojsonio::geojson_read("map_data/CC Spaulding.geojson", what = "sp")
eastway <- geojsonio::geojson_read("map_data/Eastway.geojson", what = "sp")
ek <- geojsonio::geojson_read("map_data/EK Powe.geojson", what = "sp")
fayetteville <- geojsonio::geojson_read("map_data/Fayetteville St.geojson", what = "sp")
forest <- geojsonio::geojson_read("map_data/Forest View.geojson", what = "sp")
hillside <- geojsonio::geojson_read("map_data/Hillside.geojson", what = "sp")
jordan <- geojsonio::geojson_read("map_data/CEJordan.geojson", what = "sp")
lakewood <- geojsonio::geojson_read("map_data/Lakewood.geojson", what = "sp")
parkwood <- geojsonio::geojson_read("map_data/Parkwood.geojson", what = "sp")
southwest <- geojsonio::geojson_read("map_data/Southwest.geojson", what = "sp")

#Spatial Data
bus <- read.csv("spatial_data/renamed_Bus Stops.csv")
childcare <- read.csv("spatial_data/renamed_Childcare Centers.csv")
cultural <- read.csv("spatial_data/renamed_Community & Cultural Centers.csv")
gardens <- read.csv("spatial_data/renamed_Community Gardens.csv")
grocery <- read.csv("spatial_data/renamed_Grocery Stores.csv") #note what we consider a grocery store, not food desert food apartheid
libraries <- read.csv("spatial_data/renamed_Libraries.csv")
parks <- read.csv("spatial_data/renamed_Parks.csv")
rec <- read.csv("spatial_data/renamed_Recreation Centers.csv")
religious <- read.csv("spatial_data/renamed_Religious Centers.csv")
...
body <- {dashboardBody(
    tabItems(
        ...
        {tabItem(tabName = "home",
                 fluidRow(
                     img(src="Duke.png",  style="display: block; margin-left: auto; margin-right: auto;")
                 ),
                 br(),
                 fluidRow(
                     class = "text-center",
                 ...
                 fluidRow(
                     ...
                     box(title = strong("Measurements, Context, and Resources"), box(width = 12,
                        ...
                        choices = list("Advanced Placement (AP) Course Enrollment", "Average Class Size","Bachelor Degree Rate",
                                       "CTE Enrollment Rate, High School","Diversity per School Zone", "Enrollment","ESL Students","Experienced Teacher Ratio",
